...
define('ORDER_STATUS_PENDING', 'pending'); //constant
...
if (!empty($searchQuery)) {
    $pending_orders = getAllOrdersMatchToSearchKey($mysqli_conn, $order_status, $searchQuery);
    $isSearchMode = true;
} else {
    $pagination = paginate(
        $mysqli_conn,
    ...
    $pending_orders = $pagination['items'];
                ...
                <table class="table table-bordered">
                    ...
                    <tbody>
                        <?php if (empty($pending_orders)): ?>
                            <tr>
                                <td colspan="5" class="text-center text-muted">No pending orders found.</td>
                            </tr>
                            <?php else:
