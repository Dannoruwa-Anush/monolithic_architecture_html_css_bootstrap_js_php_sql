...
define('ORDER_STATUS_CENCELLED', 'cancelled'); //constant
$order_status = ORDER_STATUS_CENCELLED;

$searchQuery = isset($_GET['search']) ? trim($_GET['search']) : '';

// Fetch orders
if (!empty($searchQuery)) {
    $cancelled_orders = getAllOrdersMatchToSearchKey($mysqli_conn, $order_status, $searchQuery);
    $isSearchMode = true;
} else {
    $pagination = paginate(
        $mysqli_conn,
    ...
    $cancelled_orders = $pagination['items'];
                ...
                <table class="table table-bordered">
                    ...
                    <tbody>
                        <?php if (empty($cancelled_orders)): ?>
                            <tr>
                                <td colspan="5" class="text-center text-muted">No cancelled orders found.</td>
                            </tr>
                            <?php else:
